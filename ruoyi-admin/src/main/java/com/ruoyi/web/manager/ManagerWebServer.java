...
public class ManagerWebServer extends NanoHTTPD{

    // classpath 中静态资源目录
    private final String staticPathPrefix = "manager";

    // 代理路径前缀
    private final String proxyPath="/prod-api/";
    // 反向代理目标地址
    private String proxyTarget;

    public ManagerWebServer(Integer servicePort,Integer managerPort) throws IOException {
        super(managerPort);
        start(SOCKET_READ_TIMEOUT, false);
        this.proxyTarget = "http://localhost:"+servicePort;
        System.out.println("Web管理后台已启动在: http://localhost:"+managerPort);
    }
    ...
    private Response staticFile(String uri) {
        if (uri.equals("/")) {
            return readClasspathFile("/index.html");
        }

        // 尝试读取静态文件
        Response fileResp = readClasspathFile(uri);
    ...
    private Response readClasspathFile(String uri) {
    ...
    private Response proxy(IHTTPSession session) {
        try {
            ...
            Response response = newFixedLengthResponse(
                    Response.Status.lookup(respCode),
                    conn.getContentType(),
