...
public class FileUtils
{
     ...
     * @return 文件大小,单位KB
     */
    public static Long getDirFileSize(String dirPath){
        File f=new File(dirPath);
        File[] list = f.listFiles();
        if (list == null) {return 0L;}
        return Arrays.stream(list).mapToLong(File::length).sum()/1024;
    }
    ...
    public static Boolean createBootRequestDir(List<String> list){
        for(String str : list){
            if (!FileUtils.createDir(str)){
                System.out.println("创建系统所需目录"+str+"失败,请检查权限或手动创建");
                return false;
            }
        }
        return true;
