...
public class SysLoginService
{
     ...
     * @return 结果
     */
    public String adminLogin(LoginBody loginBody) {
        SysUser user=userService.selectUserByUserName(loginBody.getUsername());
        if (user == null){
            throw new ServiceException(ResultCodes.USER_NOT_FOUND);
        }
        if (!user.getRoles().get(0).getRoleKey().equals("operation") && !user.getRoles().get(0).getRoleKey().equals("admin")){
            throw new ServiceException(ResultCodes.PERMISSION_ERROR);
        }
